...
<body>
          ...
          <div class="global-controls compact-config">
            ...
            <div class="block-12 global-card slim">
              <div class="global-actions">
                ...
                <button type="button" id="saveWorkspaceConfigBtn" class="secondary">保存工作区设置</button>
              </div>
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    function updateStatus(job) {
      ...
      byId("translationPreview").textContent = lastTranslation ? lastTranslation.slice(0, 280) : "-";
    }
    ...
    async function loadState() {
      try {
        ...
        setBatchStatus("error", "初始化失败", err.message || "本地服务暂时不可用");
        alert("读取本地配置失败：" + (err.message || "本地服务暂时不可用"));
      }
    }
    ...
    byId("saveWorkspaceConfigBtn").addEventListener("click", async () => {
      try {
        const data = await saveConfig();
        if (!data.ok) throw new Error(data.error || "保存失败");
        alert("工作区设置已保存。");
...
def strip_xml_text(text: str) -> str:
    ...
    text = re.sub(r"</?(?:\w+:)?(?:t|p|r|si|a|span|div|body|txBody|sheetData|row|c|is)[^>]*>", "\n", text)
    text = re.sub(r"<[^>]+>", " ", text)
    text = (
        text.replace("&lt;", "<")
        .replace("&gt;", ">")
...
def normalize_translation_text(text: str) -> str:
    ...
    for line in lines:
        if not merged:
            ...
            continue
        prev = merged[-1]
        if line in standalone_connectors:
            merged[-1] = f"{prev} {line}".strip()
            continue
        keep_heading_break = (
            prev.endswith(":")
            and len(prev.split()) <= 3
            and (re.match(r"^\d+[.)]", line) or len(line.split()) > 3)
        )
        if not keep_heading_break and (
            prev.endswith((":", "->", "-->", ",", "(", "[", "+", "/", "&"))
            or line.startswith((",", ".", ":", ";", ")", "]", "%"))
            or re.match(r"^[a-z]", line)
            or re.match(r"^(and|or|by|to|of|for|with)\b", line, flags=re.I)
        ):
            merged[-1] = f"{prev} {line}".strip()
            continue
        merged.append(line)
